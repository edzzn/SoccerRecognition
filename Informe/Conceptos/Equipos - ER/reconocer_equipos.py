...
def reconocer_equipo(image):
    ...
    lower_cian = np.array([32,78,137])
    upper_cian = np.array([78,255,255])
    ...
    for c in contours:
        ...
        if(h >= (1.1)*w):
            if(w > 0.5 and h >= 0.5):
                ...
                # Arquero RM
                conteoNoZero3 = contadorMascara(
                    jugador_img, player_hsv, lower_cian, upper_cian)
                orden = max(sorted([conteoNoZero1, conteoNoZero, conteoNoZero2,conteoNoZero3]))
                ...
                if(conteoNoZero3 >= 20 and orden == conteoNoZero3):
                    equipo = "Arquero RM"
